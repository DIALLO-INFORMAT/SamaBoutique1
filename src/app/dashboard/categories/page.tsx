// src/app/dashboard/categories/page.tsx (Manager View)
'use client';
...
const fetchCategoriesFromAPI = async (): Promise<Category[]> => {
    ...
    })).sort((a: Category, b: Category) => a.name.localeCompare(b.name)); // Sort alphabetically
};
...
const updateCategoryAPI = async (id: string, name: string, description?: string): Promise<Category> => {
    ...
    if (typeof window !== 'undefined') {
        ...
        // Check for duplicate names (excluding the current category being updated)
...
export default function ManagerCategoriesPage() {
    ...
    const handleDialogSave = async () => {
        if (!categoryName.trim()) {
            setDialogError("Le nom de la catégorie est requis.");
            return;
        }
        setDialogError('');
        setIsProcessing(editingCategory ? editingCategory.id : 'new');
    ...
    const handleDeleteCategory = async (id: string, name: string) => {
        ...
        try {
            ...
            toast({ title: t('general_error'), description: "Impossible de supprimer la catégorie.", variant: "destructive" });
        } finally {
            setIsProcessing(null);
        }
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-primary flex items-center gap-2"><FolderTree /> {t('manager_categories_page_title')}</h1>
                    <p className="text-muted-foreground">{t('manager_categories_description')}</p>
                </div>
                <Button variant="destructive" size="sm" onClick={openAddDialog}>
                    <PlusCircle className="mr-2 h-4 w-4" /> {t('manager_categories_add_button')}
                </Button>
            </div>

            <Card className="shadow-md border-border">
                <CardHeader className="border-b border-border px-6 py-4">
                    <CardTitle>{t('manager_categories_table_title')}</CardTitle>
                    {/* Optional: Search/Filter */}
                </CardHeader>
                <CardContent className="p-0">
                    {categories.length === 0 ? (
                        <p className="text-center text-muted-foreground py-12">{t('manager_categories_no_categories')}</p>
                    ) : (
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    <TableHead className="px-6">{t('manager_categories_table_name')}</TableHead>
                                    <TableHead className="px-6 hidden sm:table-cell">{t('manager_categories_table_description')}</TableHead>
                                    <TableHead className="text-right px-6 w-[100px]">{t('manager_categories_table_actions')}</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                            ...
                                            <AlertDialog>
                                                <DropdownMenu>
                                                    <DropdownMenuTrigger asChild>
                                                        <Button size="icon" variant="ghost" disabled={isProcessing === category.id}>
                                                            {isProcessing === category.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <MoreHorizontal className="h-4 w-4" />}
                                                 ...
                                                 <AlertDialogContent>
                                                    <AlertDialogHeader>
                                                        <AlertDialogTitle>{t('manager_categories_delete_confirm_title')}</AlertDialogTitle>
                                                        <AlertDialogDescription>
                                                            {t('manager_categories_delete_confirm_description', { categoryName: category.name })}
                                                        </AlertDialogDescription>
                                                    </AlertDialogHeader>
                                                    <AlertDialogFooter>
                                                        ...
                                                        >
                                                            {isProcessing === category.id && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                                                            {t('general_delete')}
                                                        </AlertDialogAction>
                                                    </AlertDialogFooter>
                                                </AlertDialogContent>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>{editingCategory ? t('manager_categories_edit_dialog_title') : t('manager_categories_add_dialog_title')}</DialogTitle>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div className="space-y-2">
                            <Label htmlFor="cat-name">{t('manager_categories_table_name')}</Label>
                            <Input
                                id="cat-name"
                                value={categoryName}
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="cat-desc">{t('manager_categories_table_description')} ({t('checkout_form_optional')})</Label>
                            <Textarea
                                id="cat-desc"
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleDialogSave} disabled={!categoryName.trim() || !!isProcessing} variant="destructive">
