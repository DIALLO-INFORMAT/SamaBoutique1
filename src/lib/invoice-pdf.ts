...
export const generateInvoicePDF = async (order: Order, t: (key: string, options?: TOptions<string>) => string): Promise<Blob> => {
  ...
  try { // Wrap generation in try-catch
    // --- Header ---
    doc.setFontSize(18);
    ...
    yPos += 5;
    doc.text(`Tel: +221 33 800 00 00 | Email: contact@samaboutique.com`, 15, yPos); // Use a placeholder email or fetch from settings
    yPos += 15;
    ...
    // Ensure autoTable is called correctly on the jsPDF instance
    (doc as any).autoTable({ // Cast to any if types are conflicting
      startY: yPos,
      head: [tableColumn],
      body: tableRows,
      theme: 'grid',
    ...
    // Get Y position after table using the correct property access
    ...
    // --- Footer ---
    yPos = pageHeight - 25 > yPos ? pageHeight - 25 : yPos + 10;
    doc.setFontSize(8);
    doc.setFont('helvetica', 'italic');
    doc.text(translate(t, 'footer_about_us'), 15, yPos);
    doc.text(`${translate(t, 'copyright')} © ${new Date().getFullYear()} ${translate(t, 'site_name')}`, 105, yPos, { align: 'center'});
    yPos += 5;
    doc.text(translate(t, 'developed_by'), 105, yPos, { align: 'center'});

  } catch (pdfError) {
      console.error("Error during PDF generation:", pdfError);
      // Create a fallback error blob or throw the error
      const errorDoc = new jsPDF();
      errorDoc.text("Erreur lors de la génération de la facture.", 10, 10);
      return errorDoc.output('blob');
      // Or: throw new Error("Failed to generate PDF invoice.");
  }
